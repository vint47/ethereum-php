...
class EthD32Left extends EthD
{
    ...
    {
        $val = EthereumStatic::removeHexPrefix($val);
        if (strlen($val) <= 64) {
            $padUp = 64 - strlen($val);
            $val = str_repeat('0', $padUp) . $val;
        }
        $val = EthereumStatic::ensureHexPrefix($val);
        if (strlen($val) === 66) {
            return $val;
        }
        else {
    ...
    public function val()
    {
//        dd(substr($this->value, 2));
//        if ($this->abi) {
//            // Shortening Data according to ABI lenngth if available.
//            dd($this->abi);
//            dd($this->getDataStrLength());
//            dd(substr($this->value, 2, $this->getDataStrLength()));
